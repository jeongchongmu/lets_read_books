...
function shallowEqual(objA: mixed, objB: mixed): boolean {
  if (is(objA, objB)) {
    // 두 객체가 같은 참조인 경우 true 반환
    return true;
  }

  if (
    // 두 객체 중에 하나라도 null, undefined 또는 객체 타입이 아닌 경우 false
    typeof objA !== 'object' ||
    objA === null ||
    typeof objB !== 'object' ||
    objB === null
...
예\_직접적으로 counter변수를 노출시키지 않음으로써 사용자가 직접 수정하는 것 막음

예\_접근하는 경우를 제한해 로그를 남김
...
- 마이크로 태스크 큐는 기존 태스크 큐보다 우선권을 갖음 (예\_`Promise`)
